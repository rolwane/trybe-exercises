...
const checkAnswers = (RIGHT_ANSWERS, STUDENT_ANSWERS) => {
  ...
  STUDENT_ANSWERS.forEach((studentAnswer, i) => {
    nota += studentAnswer === RIGHT_ANSWERS[i] ? 1 : studentAnswer === 'N.A' ? 0 : -0.5;
  });

  return nota;
}

const nota = (RIGHT_ANSWERS, STUDENT_ANSWERS, callback) => {
  return `A nota do estudante foi: ${callback(RIGHT_ANSWERS, STUDENT_ANSWERS)}`;
}

console.log(nota(RIGHT_ANSWERS, STUDENT_ANSWERS, checkAnswers));
